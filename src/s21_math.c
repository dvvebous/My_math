...
long double s21_asin(double x) {
  ...
  if (S21_IS_INF(x) || S21_IS_NAN(x))
    ...
    for (int i = 0; i < 700; i++) {
      result += ((s21_fuctorial(2 * i)) * s21_pow_int(x, 2 * i + 1)) /
                ((2 * i + 1) * s21_pow_int(2, 2 * i) *
                 s21_pow_int(s21_fuctorial(i), 2));
    }
  } else {
    result = NAN_NUM;
  }
...
long double s21_atan(double x) {
  if (S21_IS_INF(x)) return x > 0 ? S21_PI / 2 : -S21_PI / 2;
  int sign = x >= 0 ? 1 : -1;
  x *= sign;
  long double result = 0;
  if (s21_fabs(x) < 1.) {
...
long double s21_exp(double x) {
  ...
  if (s21_isnan(x)) {
    ...
    for (int i = 1; s21_fabs(q) > EPS_NUM && s21_fabs(res) < POSITIVE_INF_NUM;
         i++) {
      res += q;
      q *= x1 / i;
    }
...
long double s21_fuctorial(long long int x) {
  if (x == 1 || x == 0) return 1;
  if (x < 0) return POSITIVE_INF_NUM;
...
int s21_is_pos_inf(double x) { return x == POSITIVE_INF_NUM ? 1 : 0; }

int s21_is_neg_inf(double x) { return x == NEGATIVE_INF_NUM ? 1 : 0; }
...
long double s21_pow(double base, double exp) {
  ...
  if (base == NEGATIVE_INF_NUM && exp == POSITIVE_INF_NUM)
    return POSITIVE_INF_NUM;
...
long double s21_pow_int(long double x, int n) {
  int sign = (n > 0);
  n *= (sign) ? 1 : -1;
  long double expression = 1;
  for (int i = 0; i < n; i++) {
    expression = sign ? expression * x : expression / x;
  }
  return expression;
}
